    ...
    <x-slot name="header">
        <div class="flex flex-row items-center justify-between">
            ...
            <div class="flex flex-row gap-x-6">
                ...
                <x-modal name="confirm-user-deletion" focusable>
                    <form action="{{ route('user.destroy', $user->id) }}" method="post" class="p-6">
                        @csrf
                        @method('delete')

                        <h2 class="text-lg font-medium text-gray-900 dark:text-gray-100">
                            {{ __('Você tem certeza que deseja exclur este usuário?') }}
                        </h2>
        ...
        <div class="">
            <div class="grid grid-cols-2 gap-x-4 text-gray-400">
                <h4>Nome: <span class="font-bold">{{ __($user->name) }}</span></h4>
            </div>
        </div>
    </x-slot>
