            ...
            @if (auth()->user()->teacher && auth()->user()->teacher->isNot($publicTopic->topic->teacher))     
                <div class="flex flex-row gap-x-6">
                    <form class="flex" action="{{ $publicTopic->likes()->where('teacher_id', auth()->user()->teacher->id)->exists() ? route('unlike', $publicTopic->id) : route('like', $publicTopic->id) }}" method="POST">
                        @csrf
                        <button class="inline-flex items-center px-3 py-0 bg-white dark:bg-gray-800 border border-gray-300 dark:border-gray-500 rounded-md font-semibold text-xs text-gray-700 dark:text-gray-300 uppercase tracking-widest shadow-sm hover:bg-gray-50 dark:hover:bg-gray-700 focus:outline-none focus:ring-2 focus:ring-indigo-500 focus:ring-offset-2 dark:focus:ring-offset-gray-800 disabled:opacity-25 transition ease-in-out duration-150">
                            {!! $publicTopic->likes()->where('teacher_id', auth()->user()->teacher->id)->exists()
                    ...
                    <form action="{{ $publicTopic->teachers->contains(auth()->user()->teacher) ? route('community.destroy', $community->id) : route('community.store') }}" method="post">
                        ...
                        <x-secondary-button type="submit">{{ $publicTopic->teachers->contains(auth()->user()->teacher) ? 'Remover Tópico' : 'Adicionar Tópico' }}</x-secondary-button>
                    </form>
                </div>
            @endif
        </div>
